-- Keep derived scores consistent when a provider corrects or rolls back a
-- match result. Finished matches are recalculated from source data every run.
...
BEGIN
  ...
  WHERE p.match_id = m.id
    ...
    AND (
      ...
      OR p.is_exact_score_correct
      OR p.points_awarded <> 0
    );
