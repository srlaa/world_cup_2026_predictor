...
export function LeaguePanel() {
  ...
  return <div className="min-w-0 max-w-full overflow-hidden animate-fadeIn">
    ...
    <div className="mb-7 grid gap-4 lg:grid-cols-2">
      <form onSubmit={create} className="flex min-w-0 flex-col gap-2 rounded-2xl border border-white/10 bg-white/5 p-4 sm:flex-row"><input value={name} onChange={(e) => setName(e.target.value)} required minLength={2} maxLength={50} placeholder="League name" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-white outline-none focus:border-emerald-500/50" /><button className="flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 font-semibold text-white"><Plus className="h-4 w-4" />Create</button></form>
      <form onSubmit={join} className="flex min-w-0 flex-col gap-2 rounded-2xl border border-white/10 bg-white/5 p-4 sm:flex-row"><input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} required placeholder="Invite code" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-4 py-3 uppercase text-white outline-none focus:border-blue-500/50" /><button className="flex items-center justify-center gap-2 rounded-xl bg-blue-500 px-4 py-3 font-semibold text-white"><UserPlus className="h-4 w-4" />Join</button></form>
    </div>
    {leagues.length === 0 ? <div className="rounded-2xl border border-dashed border-white/15 py-16 text-center text-white/45">Create your first private league to start.</div> : <>
      ...
      {current && <div className="mb-5 flex min-w-0 flex-col gap-3 rounded-xl border border-white/10 bg-white/5 p-4 sm:flex-row sm:items-center"><div className="min-w-0"><p className="text-xs text-white/40">Invite code</p><p className="truncate font-mono text-lg font-bold tracking-widest text-white">{current.invite_code}</p></div><button onClick={share} className="flex w-full items-center justify-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm font-semibold text-emerald-300 sm:ml-auto sm:w-auto">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? 'Copied' : 'Copy invite link'}</button></div>}
